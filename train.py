...
# import network
from nnet.conv_tasnet_mid_feature import model_info
from nnet.conv_tasnet_mid_feature import TasNet as nnet
...
exp_name = model_name + "_" + model_info["exp_name"] + "_" + loss_type +"_1_1_unshared_stack_.5_1_1_sqrtmse1"
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        for utt, egs in enumerate(train_data_loader):
            ...
            train_est, train_est_reverb, train_est_2, train_est_reverb_2, feature_r, feature_a = torch.nn.parallel.data_parallel(model, (train_mix,train_mix_a),
                                                        device_ids=gpuids)
            ...
            batch_loss = loss_calc(train_est, train_clean, model_info['loss_type']) + \
                ...
                1 * loss_calc(train_est_2, train_clean, model_info['loss_type']) + \
                1 * loss_calc(train_est_reverb_2, train_clean_reverb, model_info['loss_type']) + \
                1 * loss_calc(feature_r, feature_a, "mse_single_channel")
        ...
        with torch.no_grad():
            for utt, egs in enumerate(valid_data_loader):
                ...
                valid_est, valid_est_reverb, valid_est_2, valid_est_reverb_2, feature_r, feature_a = torch.nn.parallel.data_parallel(model, (valid_mix, valid_mix_a),
                                                            device_ids=gpuids)

                batch_loss_anechoic = loss_calc(valid_est, valid_clean, model_info['loss_type'])
                batch_loss = batch_loss_anechoic + \
                    0.5 * loss_calc(valid_est_reverb, valid_clean_reverb, model_info['loss_type']) + \
                    1 * loss_calc(valid_est_2, valid_clean, model_info['loss_type']) + \
                    1 * loss_calc(valid_est_reverb_2, valid_clean_reverb, model_info['loss_type']) + \
                    1 * loss_calc(feature_r, feature_a, "mse_single_channel")
                loss = torch.mean(batch_loss)
