...
def loss_calc(est, ref, loss_type):
    ...
    if loss_type == "osisdr":
        loss = batch_SDR_torch(est, ref, optimal=True)
    if loss_type == "mse":
        loss = batch_mse_torch(est, ref)
    if loss_type == "log_mse":
        loss = batch_log_mse_torch(est, ref)
    if loss_type == "mse_single_channel":
        loss = batch_mse_torch_single_channel(est, ref)
    if loss_type == "log_mse_single_channel":
        loss = batch_log_mse_torch_single_channel(est, ref)


    return loss


def calc_sdr_torch(estimation, origin, mask=None, optimal=False):
    """
    ...
    est_power = torch.pow(estimation, 2).sum(1, keepdim=True) + 1e-8
    if optimal:
        scale = est_power / torch.sum(origin*estimation, 1, keepdim=True)
    else:
...
def batch_SDR_torch(estimation, origin, mask=None, optimal=False):
    """
    ...
    for i in range(nsource):
        for j in range(nsource):
            SDR[:,i,j] = calc_sdr_torch(estimation[:,i], origin[:,j], mask, optimal)
...
def batch_mse_torch_single_channel(estimation, origin):
    """
    batch-wise mse caculation for multiple audio files.
    estimation: (batch, frames, freq_bins)
    origin: (batch, frames, freq_bins)
    B N L
    """
    mse = torch.sqrt(torch.pow(estimation - origin, 2).mean([2])).mean([0,1])
    # mse = torch.pow(estimation - origin, 2).mean([0,1,2])
    return mse

def batch_log_mse_torch_single_channel(estimation, origin):
    """
    batch-wise mse caculation for multiple audio files.
    estimation: (batch, frames, freq_bins)
    origin: (batch, frames, freq_bins)
    ...
    """
    # eps = 1e-20
    log_mse = torch.log10(torch.pow(estimation - origin, 2).mean([2])).mean([0,1])
    return log_mse

if __name__ == "__main__":
    est = torch.rand(10, 32, 1000)
    ref = torch.rand(10, 32, 1000)

    out = loss_calc(est, ref, "log_mse_single_channel")
    print(out.shape)
    print(out)
